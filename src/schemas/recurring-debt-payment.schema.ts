...
export const createRecurringDebtPaymentSchema = z
  .object({
  ...
  .refine(
    (data) => {
      ...
      if (data.frequency === RECURRING_FREQUENCY.MONTHLY && !data.dayOfMonth) {
        return false;
      }
      // If frequency is weekly, dayOfWeek is required
      if (data.frequency === RECURRING_FREQUENCY.WEEKLY && data.dayOfWeek === undefined) {
        return false;
      }
      return true;
    },
    {
      message:
        'Para pagos mensuales se requiere el día del mes, para pagos semanales se requiere el día de la semana',
    }
  );
