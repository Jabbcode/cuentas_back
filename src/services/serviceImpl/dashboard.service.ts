...
import type {
  ...
  MonthlySummary,
  FixedVsVariable,
} from '../interfaces/dashboard.service.port.js';
...
export class DashboardServiceImpl implements DashboardService {
  ...
  async getSummary(userId: string): Promise<DashboardSummary> {
    try {
      ...
      return logger.fail(
        error,
        'No se pudo obtener el resumen del dashboard del usuario {}',
        userId
      );
    }
  }
  ...
  async getByCategory(
    ...
    try {
      ...
      return logger.fail(
        error,
        'No se pudo obtener el desglose por categoria del dashboard del usuario {}',
        userId
      );
    }
  }
  ...
  async getMonthlyTrend(userId: string, months = 6): Promise<MonthlyTrendItem[]> {
    try {
      ...
      return logger.fail(
        error,
        'No se pudo obtener la tendencia mensual del dashboard del usuario {}',
        userId
      );
    }
  }
  ...
  async getFixedVsVariable(userId: string): Promise<FixedVsVariable> {
    try {
      ...
      return logger.fail(
        error,
        'No se pudo obtener el comparativo de gastos fijos vs variables del usuario {}',
        userId
      );
    }
  }
