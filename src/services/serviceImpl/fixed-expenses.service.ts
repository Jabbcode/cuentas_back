...
import type { RecurringDebtPaymentsService } from '../interfaces/recurring-debt-payments.service.port.js';
...
const logger = createLogger('FIXED_EXPENSES');
...
export class FixedExpensesServiceImpl implements FixedExpensesService {
  ...
  async getFixedExpenses(userId: string, activeOnly = false): Promise<FixedExpenseWithRelations[]> {
    try {
      return (await this.fixedExpenseRepo.findAllByUser(
        userId,
        ...
        [{ dueDay: 'asc' }]
      )) as unknown as FixedExpenseWithRelations[];
    } catch (error) {
      return logger.fail(error, 'No se pudieron obtener los gastos fijos del usuario {}', userId);
    }
  }
  ...
  async getFixedExpenseById(id: string, userId: string): Promise<FixedExpenseWithTransactions> {
    try {
      ...
      return logger.fail(error, 'No se pudo obtener el gasto fijo {} del usuario {}', id, userId);
    }
  }
  ...
  async createFixedExpense(
    ...
    try {
      return (await this.fixedExpenseRepo.create(
      ...
      return logger.fail(error, 'No se pudo crear el gasto fijo del usuario {}', userId);
    }
  }
  ...
  async updateFixedExpense(
    ...
    try {
      ...
      return (await this.fixedExpenseRepo.update(id, data as Prisma.FixedExpenseUpdateInput, {
        account: { select: { id: true, name: true, color: true } },
        category: { select: { id: true, name: true, icon: true, color: true } },
      })) as unknown as FixedExpenseWithRelations;
    } catch (error) {
      return logger.fail(
        error,
        'No se pudo actualizar el gasto fijo {} del usuario {}',
        id,
        userId
      );
    }
  ...
  async deleteFixedExpense(id: string, userId: string) {
    ...
    try {
      return await this.fixedExpenseRepo.remove(id);
    } catch (error) {
      return logger.fail(error, 'No se pudo eliminar el gasto fijo {} del usuario {}', id, userId);
    }
  }
    ...
    try {
      ...
      if (fixedExpense.creditCardAccountId) {
        try {
          ...
          logger.warn(
            'El pago del estado de cuenta de la tarjeta {} del gasto fijo {} ya existia, se omite',
            fixedExpense.creditCardAccountId,
            id
          );
        }
      ...
      if (fixedExpense.recurringDebtPaymentId) {
        try {
          ...
          logger.error(
            error,
            'No se pudo registrar el pago de deuda para el gasto fijo {} del usuario {}',
            id,
            userId
          );
        }
      ...
      return logger.fail(
        error,
        'No se pudo procesar el pago del gasto fijo {} del usuario {}',
        id,
        userId
      );
    }
  ...
  async autoGenerateFixedExpenseTransactions(today: Date): Promise<AutoGenerateSummary> {
    try {
      ...
      return logger.fail(
        error,
        'No se pudieron autogenerar los gastos fijos del dia {}',
        today.toISOString()
      );
    }
  }
  ...
  async reorderFixedExpenses(
    ...
    try {
      ...
      return logger.fail(error, 'No se pudo reordenar los gastos fijos del usuario {}', userId);
    }
  }
  ...
  async getFixedExpensesSummary(userId: string): Promise<FixedExpensesSummary> {
    ...
    try {
      ...
      return logger.fail(
        error,
        'No se pudo calcular el resumen de gastos fijos del usuario {}',
        userId
      );
    }
  }
  ...
  async getActiveFixedExpenses(userId: string) {
    try {
      return await this.fixedExpenseRepo.findAllByUser(userId, { isActive: true });
    } catch (error) {
      return logger.fail(
        error,
        'No se pudieron obtener los gastos fijos activos del usuario {}',
        userId
      );
    }
  }
  ...
  async getActiveFixedExpensesWithCategory(userId: string) {
    try {
      ...
      return logger.fail(
        error,
        'No se pudieron obtener los gastos fijos activos con categoria del usuario {}',
        userId
      );
    }
  }
  ...
  async getActiveExpenseFixedExpenses(userId: string) {
    try {
      return await this.fixedExpenseRepo.findAllByUser(userId, {
        isActive: true,
      ...
      return logger.fail(
        error,
        'No se pudieron obtener los gastos fijos de tipo gasto del usuario {}',
        userId
      );
    }
  }
  ...
  async countByUser(userId: string): Promise<number> {
    try {
      return await this.fixedExpenseRepo.countByUser(userId);
    } catch (error) {
      return logger.fail(error, 'No se pudo contar los gastos fijos del usuario {}', userId);
    }
  }
    ...
    try {
      ...
      for (const card of creditCards) {
        try {
          ...
          logger.error(
            error,
            'No se pudo sincronizar el gasto fijo de la tarjeta {} del usuario {}',
            card.id,
            userId
          );
        }
      ...
      return logger.fail(
        error,
        'No se pudo sincronizar los gastos fijos de tarjetas de credito del usuario {}',
        userId
      );
    }
  }
  ...
  private async syncRecurringDebtPaymentFixedExpenses(userId: string) {
    try {
      ...
      const monthlyActive = recurringPayments.filter(
        (p) => p.isActive && p.frequency === 'monthly'
      );
      ...
      for (const payment of monthlyActive) {
        try {
          ...
          logger.error(
            error,
            'No se pudo sincronizar el gasto fijo del pago recurrente {} de deuda del usuario {}',
            payment.id,
            userId
          );
        }
      ...
      return logger.fail(
        error,
        'No se pudo sincronizar los gastos fijos de pagos recurrentes de deuda del usuario {}',
        userId
      );
    }
  }
